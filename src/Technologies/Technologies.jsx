  ...
  if (loading) {
    return (
      <div
        ...
        dir="rtl"
      >
        جاري تحميل مقالات التقنية...
      </div>
    );
  }

  // تصفية أمنة ومرنة لتجنب الأخطاء عند اختلاف المسمى (تطابق مع أي كلمة تحتوي على "تقنيات" أو "تقنية")
  const filteredProducts = products.filter((post) => {
    if (!post || !post.category) return false;
    return post.category.trim().includes("تقني");
  });
    ...
    <div dir="rtl" className="w-full pt-4 space-y-8 font-sans">
      ...
      <div className="flex justify-between items-center border-b border-neutral-800/80 pb-4">
        <div className="text-sm font-semibold text-neutral-300">
          ...
          مقالات في <strong className="text-[#FF6000]">التقنية</strong>
        </div>
      ...
      {filteredProducts.length === 0 ? (
        <div className="text-center py-16 text-neutral-400 bg-[#121212] rounded-2xl border border-neutral-800">
          لا توجد مقالات مجهزة تحت تصنيف التقنية حالياً.
        </div>
      ) : viewMode === "grid" ? (
