...
export default function Equipment() {
  ...
  // دالة الانتقال لصفحة تفاصيل المقال
  const handleArticleClick = (id) => {
    navigate(`/Blog/${id}`);
  };

  // تصفية البيانات حسب قسم المعدات
  const filteredProducts = products.filter(
    (post) => post && post.category === "معدات",
  );

  // إعادة ضبط الترقيم للصفحة الأولى عند تغير نتائج التصفية
  useEffect(() => {
    setCurrentPage(1);
    ...
    <div dir="rtl" className="w-full pt-4 space-y-8 font-sans">
      ...
      <div className="flex justify-between items-center border-b border-neutral-800/80 pb-4">
        ...
        <div className="text-sm font-semibold text-neutral-300">
          ...
          مقالات في <strong className="text-[#FF6000]">معدات</strong>
        </div>
      ...
      {filteredProducts.length === 0 ? (
        <div className="text-center py-16 text-neutral-400 bg-[#121212] rounded-2xl border border-neutral-800">
          لا توجد مقالات مجهزة تحت تصنيف المعدات حالياً.
        </div>
      ) : viewMode === "grid" ? (
